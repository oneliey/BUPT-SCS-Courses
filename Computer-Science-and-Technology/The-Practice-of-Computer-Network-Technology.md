...
1. **开课信息**：卢美莲（2025秋）
2. **推荐等级**：不太推荐
3. **授课内容**：平均一次理论课配套一次实验课。理论课就是重复讲计网已经讲过的相关内容，基本没人去。实验课则是自己用GNS3进行模拟网络拓扑配置。
4. **考核方式**：第一节课说的是平时30%+实验报告70%，实际最后却变成了五五开。
5. **作业情况**：使用`GNS3`进行路由器和交换机的模拟配置，一共有五次实验需要老师验收：
   - 路由器的基本配置
   - 静态路由配置
   - `RIP`和`OSPF`动态路由配置（要求实验报告）
   - `VLAN`配置（要求实验报告）
...
6. **成绩情况**：85
7. **心得体会**：
   - 完全不理解开课目的的一门课。按理是让同学们在学过计网理论后有一次亲身实践应用的机会，实际上却变成了在前两周被繁琐的实验环境配置安装折磨，后续则是当赛博流水线工人，每节实验课照着ppt敲繁琐的配置命令，要是发现配错了与其找问题还不如删掉重配，对任何人类都不太友好。
   - 卢美莲老师每节实验课都会考勤，大大小小总共五次实验要验收。三次小实验的考勤和验收计入平时成绩，实验3和实验4根据报告单独计算。
   - 本人实验3按照报告要求痛苦的写了将近一万字，最后却发现老师好像不怎么看报告的质量。在查得最终分数对比了周围同学后我百思不得其解，询问了老师后得知由于实验3漏了一次考勤，实验4没细看要求——ip地址末段设的不是自己学号后两位（验收的时候并未提及）被扣了大量分数。
   - 当然如果你没有漏考勤，没有晚验收，没有犯看错实验要求的低级错误，也可以获得95+的成绩。但话说回来，这学期友好的选修课那么多，为什么要选一门又麻烦又折磨的课拷打自己。
   - 据询问室友得知，如果你的老师是张海旸，那么你还必须使用`dynamips`这个上古环境进行实验。张老师只要求验收一次但是会问很多的理论细节，而且老师，验收助教和同学三方之间还疑似各自存有信息差，管理很混乱。
